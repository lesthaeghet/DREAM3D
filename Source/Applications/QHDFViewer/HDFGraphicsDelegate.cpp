...
herr_t HDFGraphicsDelegate::_convertToImage(hid_t fileId, const std::string &datasetPath, bool withNormalization)
{
  ...
  if (err < 0)
  {
    std::cout  << "Error Getting datasetInfo." << std::endl;
    return err;
  }
  hid_t typeId = 0;
  ...
  if (typeId < 0)
  {
    std::cout  << "HDFGraphicsDelegate::_convertToImage ERROR: Unable to retrieve type info for " << datasetPath << std::endl;
    return -1;
  }
  ...
  switch(ds_type)
  {
    case H5T_STRING:
      ...
      std::cout << "Attempting to change string data into image data" << std::endl;
      break;
    case H5T_INTEGER:
